...
After the installation completes, reboot the system and log in as `eal`.
...
We add the repository now so the package is available for installation in Phase 2.
    ...
    sudo dnf install https://yum.osc.edu/ondemand/3.1/ondemand-release-web-3.1-1.el9.noarch.rpm

### F. Configure SSL Certificates (Self-Signed)

Open OnDemand requires HTTPS. We generate a self-signed certificate now to avoid configuration errors later. Note that we must use sudo to write to the protected `/etc/pki` directories.
