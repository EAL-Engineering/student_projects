...
Edit the file to match the settings below.
    ...
    Critical: Ensure ipaddr matches your bond0 static IP (10.0.0.91).

    Critical: We will disable internal DHCP management in the configuration to prevent conflicts with the Endian Firewall.
...
```Bash
# Enable the web server (OpenHPC usually pulls in httpd or nginx)
systemctl enable --now httpd

# Enable TFTP (Socket activated)
systemctl enable --now tftp.socket

# Enable Warewulf daemon (if applicable for background tasks)
