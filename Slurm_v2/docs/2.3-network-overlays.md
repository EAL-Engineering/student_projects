...
**Note:** The `BONDING_OPTS` line specifies Mode 4 (802.3ad) and a Monitoring Interval of 100ms.
...
Insert the following content:
        ...
        TYPE=Bond
        BONDING_MASTER=yes
        IPADDR=10.0.0.x  # Note: Warewulf template variables will handle this automatically
        PREFIX=16
        ONBOOT=yes
        BOOTPROTO=none
...
**Templating Note:** Warewulf allows dynamic IP injection. However, for the overlay, we usually rely on Warewulf's internal network handling to overwrite the specific IP/Mask lines, or we can use Warewulf template tags (e.g., `{{ .Ipaddr }}`) if we convert this file to a template (`.ww`). For this phase, we will let NetworkManager aggregate the links, while Warewulf assigns the IP to the interface defined in the node configuration.
