...
Now that we have finished the installation and configuration of the cluster software stack, we must validate that the system meets the functional and performance benchmarks defined in the Project Plan. This phase serves as the final "Commissioning" step before opening the system to general users.
...
* **Sum** Bandwidth: **18.5 – 19.8 Gbits/sec**
...
### 4.3 Web Portal Validation (Open OnDemand)

Verify that the web interface is correctly communicating with the scheduler.

1. **Access:** Open `https://10.0.0.91` in a browser.
2. **Login:** Use the standard user credentials.
3. **Check Queue:** Navigate to **Jobs -> Active Jobs**.
    * *Success:* You should see the `sub_talys` job you ran in step 4.2 listed in the history (Completed).
    * *Failure:* If you see a "Server Error" or "Connection Refused," check that `/etc/slurm/slurm.conf` is readable by the `ondemand` user.
