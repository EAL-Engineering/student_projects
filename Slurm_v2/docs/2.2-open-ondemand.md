...
### 2.1 Remove Conflicting Services

The `ondemand-dex` package (OpenID Connect Provider) is often installed as a weak dependency. If present, it attempts to auto-configure itself and override our PAM settings. We explicitly remove it to prevent conflicts.

    dnf remove ondemand-dex
